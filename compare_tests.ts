...
const EXTRA_CASES: { name: string; code: string; stdin?: string }[] = [
    ...
    { name: "최상위 돌려주자: 프로그램 끝", code: "\"전\"을 출력하자\n1을 돌려주자\n\"후\"를 출력하자\n" },
    { name: "타입 이름 값: 설계와 기본 타입", code: "[동물]을 설계하자:\n    '이름'을 1로 정하자\n[동물]를 출력하자\n['x', [참]]를 출력하자\n([숫자] == \"숫자\")를 출력하자\n'문자열'을 5로 정하자\n[문자열]을 출력하자\n" },
    { name: "비우기: 설계의 메서드와 문자열", code: "[통]을 설계하자:\n    '양'을 3으로 정하자\n    <비우기>를 만들자 ():\n        '나'의 '양'을 0으로 정하자\n'통'을 새로운 [통]()으로 정하자\n'통'의 <비우기>()를 실행하자\n('통'의 '양')을 출력하자\n\"글\"의 <비우기>()를 실행하자\n" },
    { name: "비우기: 인자 개수", code: "'목'을 [1]로 정하자\n'목'의 <비우기>(1)를 실행하자\n" },
    { name: "멤버 키 오류: 문자열", code: "'x'를 \"가나\"로 정하자\n일단 해보자:\n    ('x'의 없는이름)을 출력하자\n오류가 발생했다면 ('e'):\n    ('e'의 '메시지')를 출력하자\n('x'의 \"a\")을 출력하자\n" },
    { name: "멤버 키 오류: 목록 읽기·쓰기", code: "'x'를 [1, 2]로 정하자\n일단 해보자:\n    ('x'의 '없는이름')을 출력하자\n오류가 발생했다면 ('e'):\n    ('e'의 '메시지')를 출력하자\n'x'의 (1 / 0)을 9로 정하자\n" },
    { name: "최상위 돌려주자: 마무리는 돈다", code: "일단 해보자:\n    2를 돌려주자\n마무리는 항상:\n    \"마무리\"를 출력하자\n\"뒤\"를 출력하자\n" },
    ...
    { name: "연산: 소수로 나눈 나머지는 0으로 나누기", code: "(7 % 0.5)를 출력하자\n" },
    { name: "분기: 맞는 경우가 없으면 아무 일도 없음", code: "'x'를 [문자열]인 \"수\"로 정하자\n'x'에 따라 나누자:\n    \"월\" 인 경우:\n        \"가\"를 출력하자\n" },
    { name: "분기: 숫자에서 맞는 경우가 없으면 아무 일도 없음", code: "'x'를 [숫자]인 3으로 정하자\n'x'에 따라 나누자:\n    1 인 경우:\n        \"가\"를 출력하자\n    2 인 경우:\n        \"나\"를 출력하자\n" },
    { name: "분기: 나머지는이 있으면 오류가 없음", code: "'x'를 [문자열]인 \"수\"로 정하자\n'x'에 따라 나누자:\n    \"월\" 인 경우:\n        \"가\"를 출력하자\n    나머지는:\n        \"나\"를 출력하자\n" },
    { name: "반복: 문자열 순회", code: "'인사'를 [문자열]인 \"안녕😀\"으로 정하자\n'인사'의 '글자'마다 반복하자:\n    '글자'를 출력하자\n" },
    { name: "반복: 반복 변수와 끝값을 바꿔도 횟수는 그대로", code: "'끝'을 [숫자]인 3으로 정하자\n1부터 '끝'까지 반복하자 ('가'):\n    '가'를 출력하자\n    '끝'에 10을 더하자\n    '가'에 5를 더하자\n" },
    // 반복 변수와 처리기의 오류 변수는 자기 범위에 새로 선언되어 같은 이름의 바깥 변수를 가린다 (스펙 1.1)
    { name: "범위: 반복 변수가 같은 이름의 바깥 변수를 가림", code: `'가'를 100으로 정하자
1부터 2까지 반복하자 ('가'):
    '가'를 출력하자
'가'를 출력하자
` },
    { name: "범위: 범위의 끝은 가려지기 전의 바깥 변수", code: `'가'를 3으로 정하자
1부터 '가'까지 반복하자 ('가'):
    '가'를 출력하자
'가'를 출력하자
` },
    { name: "범위: 가려진 바깥 상수와 같은 이름의 반복 변수는 바꿀 수 있음", code: `'가'를 "글"로 고정하자
1부터 2까지 반복하자 ('가'):
    '가'에 10을 더하자
    '가'를 출력하자
'가'를 출력하자
` },
    { name: "범위: 가려진 상수 목록과 같은 이름의 항목에 추가할 수 있음", code: `'목'을 [1]로 고정하자
[[5]]의 '목'마다 반복하자:
    '목'에 6을 추가하자
    '목'을 출력하자
'목'을 출력하자
` },
    { name: "범위: 마다의 항목이 바깥 변수를 가림", code: `'과일'을 "원래"로 정하자
["사과", "배"]의 '과일'마다 반복하자:
    '과일'을 출력하자
'과일'을 출력하자
` },
    { name: "범위: 처리기의 오류 변수가 바깥 변수를 가림", code: `'에러'를 "원래"로 정하자
일단 해보자:
    새로운 [오류]("실패")를 발생시키자
오류가 발생했다면 ('에러'):
    '에러'의 '메시지'를 출력하자
'에러'를 출력하자
` },
    { name: "범위: 오류로 빠져나온 반복의 변수는 사라짐", code: `'가'를 100으로 정하자
일단 해보자:
    1부터 3까지 반복하자 ('가'):
        '임시'를 5로 정하자
        새로운 [오류]("멈춤")를 발생시키자
...
'임시'를 출력하자
` },
    // 반복 밖에서 실행된 반복을 끝내자는 IllegalBreakError (스펙 4.4)
    { name: "끝내기: 반복 밖에서 실행되면 IllegalBreakError", code: `"가"를 출력하자
반복을 끝내자
"나"를 출력하자
` },
    { name: "끝내기: 반복 밖이어도 실행되지 않으면 괜찮음", code: `만약 (1 == 2) 라면:
    반복을 끝내자
"끝"을 출력하자
` },
    { name: "끝내기: 함수 안의 반복을 끝내자는 부른 쪽의 반복을 끝내지 않음", code: `<끊기>를 만들자 ():
    "f"를 출력하자
    반복을 끝내자
1부터 3까지 반복하자 ('수'):
    '수'를 출력하자
...
"끝"을 출력하자
` },
    { name: "끝내기: 일단 해보자 안의 반복", code: `일단 해보자:
    1부터 5까지 반복하자 ('수'):
        만약 ('수' == 2) 라면:
            반복을 끝내자
        '수'를 출력하자
...
"끝"을 출력하자
` },
    { name: "끝내기: 처리기에서 반복을 끝내면 마무리는 한 번", code: `1부터 3까지 반복하자 ('수'):
    일단 해보자:
        새로운 [오류]("가")를 발생시키자
    오류가 발생했다면 ('e'):
        반복을 끝내자
...
"끝"을 출력하자
` },
    { name: "오류: 처리기 안의 오류도 마무리를 실행", code: `일단 해보자:
    일단 해보자:
        새로운 [오류]("가")를 발생시키자
    오류가 발생했다면 ('e'):
        새로운 [오류]("나")를 발생시키자
...
마무리는 항상:
    "F밖"을 출력하자
` },
    { name: "범위: 끝이 숫자가 아니면 RangeMustBeNumbers", code: `'n'을 "다섯"으로 정하자
1부터 'n'까지 반복하자 ('수'):
    '수'를 출력하자
` },
    { name: `반복: 목록 참조: 함수가 받은 목록에 추가하고 바꾸면 원본이 바뀜`, code: `<채우기>를 만들자 ([(숫자)목록]인 '목'):
    '목' 뒤에 99를 추가하자
    '목'의 1번째를 7로 정하자
...
'가'를 출력하자
` },
    { name: `반복: 목록 참조: 함수가 받은 목록에서 꺼내면 원본이 바뀜`, code: `<꺼내기>를 만들자 ([(숫자)목록]인 '목'):
    '목' 뒤에서 꺼내자
...
'가'를 출력하자
` },
    { name: `반복: 목록 참조: 함수가 받은 목록을 비우면 원본이 비워짐`, code: `<지우기>를 만들자 ([(숫자)목록]인 '목'):
    '목'의 <비우기>()를 실행하자
...
'가'를 출력하자
` },
    { name: `반복: 목록 참조: 같은 목록을 받은 두 변수는 하나를 공유함`, code: `'가'를 [(숫자)목록]인 [1]로 정하자
'나'를 [(숫자)목록]인 '가'로 정하자
'나' 뒤에 2를 추가하자
'가'를 출력하자
` },
    { name: `반복: 목록 참조: 목록 속의 목록`, code: `'표'를 [(목록)목록]인 [[1], [2]]로 정하자
'안'을 [(숫자)목록]인 '표'의 1번째로 정하자
'안' 뒤에 5를 추가하자
'표'를 출력하자
` },
    { name: `반복: 목록 참조: 객체의 목록 필드는 객체를 통해 공유됨`, code: `[상자]를 설계하자:
    '안'을 [(숫자)목록]인 [1]로 정하자
...
'다른통'의 '안'을 출력하자
` },
    { name: `반복: 목록 참조: 반복은 시작할 때의 목록을 돎`, code: `'수들'을 [(숫자)목록]인 [1, 2, 3]으로 정하자
'수들'의 '수'마다 반복하자:
    '수들' 뒤에 '수'를 추가하자
'수들'을 출력하자
` },
    { name: `반복: 목록 참조: 타입이 안 맞는 추가는 거절되고 목록은 그대로`, code: `'가'를 [(숫자)목록]인 [1]로 정하자
일단 해보자:
    '가' 뒤에 "글"을 추가하자
오류가 발생했다면 ('e'):
    "실패"를 출력하자
'가'를 출력하자
` },
    { name: `반복: 목록 참조: 상수 목록에는 추가할 수 없음`, code: `'목록'을 [(숫자)목록]인 [1, 2]로 고정하자
'목록' 뒤에 3을 추가하자
` },
    { name: `반복: 목록 참조: 상수 목록에서는 꺼낼 수 없음`, code: `'목록'을 [(숫자)목록]인 [1, 2]로 고정하자
'목록' 뒤에서 꺼내자
` },
    { name: "반복: 숫자는 순회할 수 없음", code: "'n'을 [숫자]인 3으로 정하자\n'n'의 '글자'마다 반복하자:\n    '글자'를 출력하자\n" },
];

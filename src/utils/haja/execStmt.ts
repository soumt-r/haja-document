...
export async function executeStmt(i: HajaInterpreter, stmt: ast.Statement, env: Environment): Promise<void> {
  switch (stmt.type) {
    case "ThrowStatement": {
    ...
    case "InputStatement": {
      const typeAnn = stmt.typeRef ? stmt.typeRef.name : "문자열";
      ...
      if (i.inputCallback) {
        ...
        userInput = (typeof prompt === "function" ? prompt(`입력 (${typeAnn}): `) : "") || "";
      }

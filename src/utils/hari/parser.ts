...
// can reuse it), this repo hardwires hariProfile directly — see
// langProfile.ts's header comment for why.
import * as ast from "./ast";
import * as tok from "./token";
import type { Token } from "./token";
import { Lexer } from "./lexer";
import { hariProfile, literalIn, LoopKind, type Component, type LangProfile } from "./langProfile";
...
export class Parser {
  ...
  private lang: LangProfile = hariProfile;
    ...
    while (this.peek() !== null && this.peek()!.type !== tok.COLON && this.peek()!.type !== tok.DEDENT && this.peek()!.type !== tok.INDENT) {
      ...
      if (t.type === tok.TYPE || t.type === tok.LBRACKET) {
        ...
        if (this.peek() && this.peek()!.type === tok.KW_BASE) {
          ...
          nameNode = tNode;
          isAbstract = this.consume().literal.startsWith("밑");
          break;
        } else {
          nameNode = tNode;
        }

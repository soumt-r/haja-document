...
산술 연산자의 결합 순서는 일반 수학과 같아요. `*` `/` `%`가 `+` `-`보다 먼저 묶이고, 같은 단계의 연산자는 왼쪽부터 묶여요. 그래서 `2 + 3 * 4`는 `left: 2, operator: "+", right: (3 * 4)`인 트리가 돼요. 소괄호로 묶은 식은 그 자체가 하나의 항이에요.
...
현재 하자·카나데 구현은 항목 목록과 `전부` 여부를 이렇게 가져요. 항목은 `<a>와 <b>를 가져오자`처럼 조사로 이어 쓰고(카나데는 `と`), 별칭(`<올림>을 <반올림>으로`)은 항목이 하나일 때만 붙일 수 있어요. `[모듈]에서 전부 가져오자`는 그 모듈이 선언한 함수·클래스·인터페이스(내장 모듈은 함수 전부)를 가져와요.

타입 리터럴(`[이름]`)로 쓴 모듈은 표준 라이브러리, Hana 런타임과 함께 배포하는 패키지, 설치한 서드파티 패키지 중 하나예요. 서드파티 패키지의 이름은 `[github.com/주인/저장소]`처럼 git 경로이고, 어느 쪽인지는 모듈을 불러오는 쪽(로더)이 정해요. 파서는 `[이름]`을 모두 `builtin`으로 읽어요. 가져온 함수가 그 모듈 안의 다른 함수를 부를 수 있다는 것(모듈 범위)은 [런타임 명세](/docs/tech-spec/3-runtime) 4.3에 있어요.
